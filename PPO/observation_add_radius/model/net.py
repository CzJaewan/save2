...
class CNNPolicy(nn.Module):
    def __init__(self, frames, action_space):
        ...
        self.act_fc2 =  nn.Linear(256+2+2+1, 128)
        ...
        self.crt_fc2 = nn.Linear(256+2+2+1, 128)
        self.critic = nn.Linear(128, 1)

    def forward(self, x, goal, speed, radius):
        """
        ...
        a = torch.cat((a, goal, speed, radius), dim=-1)
        ...
        v = torch.cat((v, goal, speed, radius), dim=-1)
        v = F.relu(self.crt_fc2(v))
        v = self.critic(v)

        return v, action, logprob, mean

    def evaluate_actions(self, x, goal, speed, radius, action):
        v, _, _, mean = self.forward(x, goal, speed, radius)
        logstd = self.logstd.expand_as(mean)
        std = torch.exp(logstd)
        # evaluate
