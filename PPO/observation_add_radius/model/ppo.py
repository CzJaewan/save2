...
def transform_buffer(buff):
    ...
    v_batch = [], [], [], [], [], [], [], [], []
    s_temp, goal_temp, speed_temp, radius_temp = [], [], [], []
    ...
    radius_batch = np.asarray(radius_batch)
    ...
    return s_batch, goal_batch, speed_batch, radius_batch, a_batch, r_batch, d_batch, l_batch, v_batch


def generate_action(env, state_list, policy, action_bound):
    if env.index == 0:
        s_list, goal_list, speed_list, radius_list = [], [], [], []
        for i in state_list:
            s_list.append(i[0])
            goal_list.append(i[1])
            speed_list.append(i[2])
            radius_list.append(i[3])
...
def ppo_update_stage1(policy, optimizer, batch_size, memory, epoch,
    ...
    obss, goals, speeds, radiuss, actions, logprobs, targets, values, rewards, advs = memory
    ...
    radiuss = radiuss.reshape((num_step*num_env, 1))
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            ...
            sampled_radiuss = Variable(torch.from_numpy(radiuss[index])).float().cuda()
            ...
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_goals, sampled_speeds, sampled_radiuss, sampled_actions)
