...
class CNNPolicy(nn.Module):
    def __init__(self, frames, action_space):
        ...
        self.act_fc2 =  nn.Linear(256+2+2+3, 128)
        ...
        self.crt_fc2 = nn.Linear(256+2+2+3, 128)
        self.critic = nn.Linear(128, 1)

    def forward(self, x, goal, speed, point):
        """
        ...
        a = torch.cat((a, goal, speed, point), dim=-1)
        ...
        v = torch.cat((v, goal, speed, point), dim=-1)
        v = F.relu(self.crt_fc2(v))
        v = self.critic(v)

        return v, action, logprob, mean

    def evaluate_actions(self, x, goal, speed, point, action):
        v, _, _, mean = self.forward(x, goal, speed, point)
        logstd = self.logstd.expand_as(mean)
        std = torch.exp(logstd)
        # evaluate
