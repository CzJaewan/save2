...
if not os.path.exists('./log/' + hostname + "_add_point"):
    os.makedirs('./log/' + hostname + "_add_point")
ppo_file = './log/' + hostname + "_add_point" + '/ppo.log'
...
def transform_buffer(buff):
    s_batch, goal_batch, speed_batch, point_batch, a_batch, r_batch, d_batch, l_batch, \
    v_batch = [], [], [], [], [], [], [], [], []
    s_temp, goal_temp, speed_temp, point_temp = [], [], [], []
    ...
    point_batch = np.asarray(point_batch)
    ...
    return s_batch, goal_batch, speed_batch, point_batch, a_batch, r_batch, d_batch, l_batch, v_batch


def generate_action(env, state_list, policy, action_bound):
    if env.index == 0:
        s_list, goal_list, speed_list, point_list = [], [], [], []
        for i in state_list:
            s_list.append(i[0])
        ...
        point_list = Variable(torch.from_numpy(point_list)).float().cuda()


        v, a, logprob, mean = policy(s_list, goal_list, speed_list, point_list)
...
def generate_action_no_sampling(env, state_list, policy, action_bound):
    if env.index == 0:
        ...
        _, _, _, mean = policy(s_list, goal_list, speed_list, point_list)
...
def ppo_update_stage1(policy, optimizer, batch_size, memory, epoch,
    ...
    obss, goals, speeds, points, actions, logprobs, targets, values, rewards, advs = memory
    ...
    points = points.reshape((num_step*num_env, 3))
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            ...
            sampled_points = Variable(torch.from_numpy(points[index])).float().cuda()
            ...
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_goals, sampled_speeds, sampled_points, sampled_actions)
