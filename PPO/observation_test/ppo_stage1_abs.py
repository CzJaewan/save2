...
from model.net_abs import MLPPolicy, CNNPolicy
from stage_world_abs import StageWorld
from model.ppo_abs import ppo_update_stage1, generate_train_data
from model.ppo_abs import generate_action
from model.ppo_abs import transform_buffer
...
def run(comm, env, policy, policy_path, action_bound, optimizer):
    ...
    for id in range(MAX_EPISODES):
        ...
        goal = np.asarray(env.goal_point)
        speed = np.asarray(env.get_self_speed())
        point = np.asarray(env.get_self_state())

        state = [obs_stack, goal, speed, point]
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            goal_next = np.asarray(env.goal_point)
            speed_next = np.asarray(env.get_self_speed())
            point_next = np.asarray(env.get_self_state())

            state_next = [obs_stack, goal_next, speed_next, point_next]
            ...
            if env.index == 0:
                ...
                if len(buff) > HORIZON - 1:
                    s_batch, goal_batch, speed_batch, point_batch, a_batch, r_batch, d_batch, l_batch, v_batch = \
                    ...
                    memory = (s_batch, goal_batch, speed_batch, point_batch, a_batch, l_batch, t_batch, v_batch, r_batch, advs_batch)
...
if __name__ == '__main__':
    ...
    output_file = './log/' + hostname + "_add_point" + '/output.log'
    cal_file = './log/' + hostname + "_add_point" + '/cal.log'
    ...
    if rank == 0:
        ...
        file = policy_path + '/abs_23000.pth'
